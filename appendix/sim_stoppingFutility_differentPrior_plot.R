# ==============================================================================
# Evaluation of simulation results: Stopping for futility with different prior
# ==============================================================================
...
SIM_numbers <- substr(SIM_numbers, 1, nchar(SIM_numbers)-21)

# Load all simulation results
lapply(SIM_names, load, .GlobalEnv) # This may take a few seconds

# Initiate results matrix
simresults <- matrix(NA, nrow = length(SIM_stopfutil_00_differentPrior[[1]]), 
...
for(i in 1:(length(SIM_names))){
  ...
  sim <- get(paste0("SIM_stopfutil_", SIM_numbers[i], "_differentPrior"))[[1]]
  SIM_bounds <- get(paste0("SIM_stopfutil_", SIM_numbers[i], "_differentPrior"))[[2]]$boundary
  SIM_n.max <- get(paste0("SIM_stopfutil_", SIM_numbers[i], "_differentPrior"))[[2]]$n.max
...
ggplot(simresults, aes(x = as.factor(finalN_00), fill = factor(reasonEnd_00))) +
  ...
  labs(x = "Sample Size per Group",
       y = "% Ending At") +
  scale_y_continuous(limits = c(0, 30), breaks = seq(0, 300, by = 50), labels = seq(0, 30, by = 5)) 
