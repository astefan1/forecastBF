# ==============================================================================
# Evaluation of simulation results: Stopping for futility with small N
# ==============================================================================
...
SIM_numbers <- substr(SIM_numbers, 1, nchar(SIM_numbers)-13)

# Load all simulation results
lapply(SIM_names, load, .GlobalEnv) # This may take a few seconds

# Initiate results matrix
simresults <- matrix(NA, nrow = length(SIM_stopfutil_00_smallN[[1]]), 
...
for(i in 1:(length(SIM_names))){
  ...
  sim <- get(paste0("SIM_stopfutil_", SIM_numbers[i], "_smallN"))[[1]]
  SIM_bounds <- get(paste0("SIM_stopfutil_", SIM_numbers[i], "_smallN"))[[2]]$boundary
  SIM_n.max <- get(paste0("SIM_stopfutil_", SIM_numbers[i], "_smallN"))[[2]]$n.max
...
simresults[, "reasonEnd_00"] <- factor(simresults[, "reasonEnd_00"], levels = c("H1", "futility", "maxN", "H0"), ordered = TRUE)
...
ggplot(simresults, aes(x = as.factor(finalN_00), fill = factor(reasonEnd_00))) +
  ...
  labs(x = "Sample Size per Group",
       y = "% Ending At") +
  scale_y_continuous(limits = c(0, 300), breaks = seq(0, 300, by = 50), labels = seq(0, 30, by = 5))  +
